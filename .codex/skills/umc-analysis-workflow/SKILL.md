...
description: "UMC 분석 작업에 사용한다. Part 1 지수/그림, Part 2 HLM 다층분석, 텍스트 전처리·분류, 추론 라운드, 타당성 점검, 보고서 전달, 분석 저장소 동기화를 포함한다."
---
...
- cmux 워커를 사용할 때는 `$umc-cmux-worker-supervision`과 함께 쓴다.
...
3.3절 방법론, 에이전트 운용, 프롬프트 세부사항, 독자용 파이프라인 설명이 필요하면 전처리와 추론 계층을 모두 확인한다. 공개 가능한 부록 후보는 키워드 요약, 분류 프롬프트 요약, 추론 프롬프트 요약, 판단 합성기 요약이다. 사용자가 프롬프트를 "as-is", "verbatim", "그대로" 요청하면 원천 프롬프트 파일을 사용하고 줄바꿈과 출처 경로 키를 보존한다.

원문 게시물, 게시물 수준 식별자, 로컬 원자료 경로, 전체 게시물 수준 LLM 덤프를 보고서에 붙여 넣지 않는다. 전체 키워드 목록이 필요하면 본문 DOCX를 혼잡하게 만들지 말고 별도 부록 산출물을 제안한다.
